...
if (isset($_POST['upload'])) { // If isset upload button or not
	...
	*/
	...
	if ($file_size > 10485760) { // Check file size 10mb or not
		echo "<script>alert('Diqqət!! Faylın ölçüsü 10 mb-dan çox ola bilməz!!')</script>";
	} else {
		...
		if ($result) {
			...
			echo "<script>alert('Fayl Sistemə uğurla upload edildi!!')</script>";
			...
			echo "<script>alert('Error!! Səhv Baş verdi')</script>";
		}
	}
}
...
<head>
	...
	<title>Fayl Upload Sistemi - Rzayev Taleh</title>
</head>
<body>
	<div class="file__upload">
		<div class="header">
			<p><i class="fa fa-cloud-upload fa-2x"></i><span><span>Up</span>loadAze.Ml</span></p>			
		</div>
		<form action="" method="POST" enctype="multipart/form-data" class="body">
			...
			<label for="link_checkbox" style="<?php echo $link_status; ?>">Link</label>

			<input type="file" name="file" id="upload" required>
			<label for="upload">
				<i class="fa fa-file-text-o fa-3x"></i>
				<p>
					<strong>Faylı Yüklə</strong> 
